...
selected_player_name = st.selectbox("Select Player", player_names, index=list(player_names).index("Lionel Messi") if "Lionel Messi" in player_names else 0)
...
player_transfers = transfers[transfers['player_id'] == player_id].sort_values('transfer_date')
...
current_val = player_vals.iloc[-1]['market_value_in_eur'] if not player_vals.empty else 0
...
with col3:
    st.metric("Total Fees Paid (Career)", f"€{total_fees:,.0f}", 
              delta=f"{(current_val - total_fees):,.0f} ROI" if total_fees > 0 else None,
              delta_color="normal")

st.subheader("Transfer History")
st.dataframe(
    player_transfers[['transfer_date', 'from_club_name', 'to_club_name', 'transfer_fee', 'market_value_in_eur']]
    .rename(columns={
        'transfer_date': 'Date', 
        'from_club_name': 'From', 
        'to_club_name': 'To',
