...
from src.data_loader import load_players, load_appearances, load_events, load_clubs
...
selected_player_name = st.selectbox("Select Player", player_names, index=list(player_names).index("Cristiano Ronaldo") if "Cristiano Ronaldo" in player_names else 0)
...
with col1: st.metric("Total Goals", int(total_goals))
with col2: st.metric("Total Assists", int(total_assists))
with col3: st.metric("Minutes Played", f"{int(total_mins):,}")
with col4: st.metric("Total Cards", int(total_cards))
...
with c1:
    ...
    if not p_apps.empty:
        ...
        fig_season = px.bar(yearly_stats, x='year', y=['goals', 'assists'], 
                            barmode='group', title="Goals & Assists by Year")
...
with c2:
    ...
    if not p_events.empty:
        ...
        fig_pie = px.pie(event_counts, values='Count', names='Event Type', hole=0.4, title="Event Distribution")
